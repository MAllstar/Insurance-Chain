...
import { Records } from '../Table/owntable';
...
const carId = name=="莫四七"?'3308******7655':'3308******7685';
const phone = name=="莫四七"?'17376591866':'17376597859';
const money = name=="莫四七"?4890:32456;
const cost = name=="莫四七"?'交强险 300,180,230,100,800,1120,50,880,230,300,300,400 正常，轻微受损，轻微受损，正常，中度受损，严重受损，严重受损，正常，损坏，轻微受损，中度受损，损坏 双方均有过错,按过错程度和对事故发生的重要性共同承担':'第三责任险 3700,1080,2330,1100,500,120,560,780,630,700,900,780 损坏，严重受损，严重受损，轻微受损，中度受损，严重受损，严重受损，正常，损坏，轻微受损，中度受损，损坏 只存在该方过错,承担全部责任';
var str = cost.split(" ");  //字符串分割，str为字符串数组
...
const contentListNoTitle = {
  owner: <p>{cookies.get('login')=='0'?null:name}</p>,
  carId: <p>{cookies.get('login')=='0'?null:carnum}</p>,
  instance: <p>{cookies.get('login')=='0'?null:str[0]}</p>,
  result: <p>{cookies.get('login')=='0'?null:str[3]}</p>
};
      ...
      <div>
        ...
        <Card style={{ width: 300, marginTop: 16 }} loading={loading}>
          <Meta
            ...
            title="个人理赔信息"
            description={<p>车主姓名: <span>{name}</span><br /> 身份证：{carId}<br />手机号：{phone}<br />
            理赔金额：<span style={{fontSize: 18,color: "red"}}>{money}</span><br />保险：{str[0]}<br />账户地址: <span style={{wordBreak: "break-all"}}>{this.context.accounts[0]}</span></p>}
          />
        </Card>
        </div>
    );
        ...
        return (
            <div className="container">
                <div className="left lf">
                    ...
                    <div>
                        <h1 className="left marginlf" ><b>设备信息</b></h1> <span style={{paddingLeft: "20px"}}>车牌号:</span> <Dropdown overlay={menu} style={{fontSize: 18}}><Button>{cookies.get('number')=='null'?"":carnum} <Icon type="down" /></Button></Dropdown>
