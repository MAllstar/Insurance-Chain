     ...
     componentDidMount  = async () => { //在第一次渲染后调用,componentDidMount 
      ...
      for(var i=bId;i<=this.context.blockId;i++){
        ...
        await instance.methods.application(i).call({from: accounts[0]},(error,result)=>{ 
          ...
          else if(result[7]=="申请中") flag = false;
        });
        if(!flag) continue;
        ...
        if(this.props.value!=''){
          ...
          return (
            <div>
              ...
              columns={columns} dataSource={cookies.get('userName')!='admin'?[]:this.dataSearch} pagination={false} bordered scroll={{x: 60, y: 180 }} expandedRowRender={record => <p style={{ margin: 0, color: "red"}}>Transaction Hash: {record.transaction}</p>}/>
            </div>
          );
        }
          ...
          return (
            <div>
                ...
                loading={this.state.load} columns={columns} dataSource={cookies.get('userName')!='admin'?[]:this.state.data} pagination={false} bordered scroll={{x: 500, y: 500 }} expandedRowRender={record => <p style={{ margin: 0, color: "red"}}>Transaction Hash: {record.transaction}</p>}/>
            </div>
          );
        }
    }
